...
func (c *PostgresCacher) hash(key string) string {
	hash := blake2b.Sum512([]byte(key))
...
func (c *PostgresCacher) Put(key string, val interface{}, expire int64) error {
	...
	now := time.Now().Unix()
	if c.IsExist(key) {
		_, err = c.c.Exec("UPDATE cache SET data=$1, created=$2, expire=$3 WHERE key=$4", data, now, expire, c.hash(key))
	} else {
		_, err = c.c.Exec("INSERT INTO cache(key,data,created,expire) VALUES($1,$2,$3,$4)", c.hash(key), data, now, expire)
	}
	return err
}
...
func (c *PostgresCacher) read(key string) (*cache.Item, error) {
	...
	err := c.c.QueryRow("SELECT data,created,expire FROM cache WHERE key=$1", c.hash(key)).Scan(&data, &created, &expire)
	if err != nil {
		return nil, err
	}
...
func (c *PostgresCacher) Delete(key string) error {
	_, err := c.c.Exec("DELETE FROM cache WHERE key=$1", c.hash(key))
	return err
}
...
func (c *PostgresCacher) IsExist(key string) bool {
	var data []byte
	err := c.c.QueryRow("SELECT data FROM cache WHERE key=$1", c.hash(key)).Scan(&data)
...
func (c *PostgresCacher) Ping() error {
	return c.c.Ping()
}
