...
func (c *MysqlCacher) hash(key string) string {
...
func (c *MysqlCacher) Put(key string, val interface{}, expire int64) error {
	...
	now := time.Now().Unix()
	if c.IsExist(key) {
		_, err = c.c.Exec("UPDATE cache SET data=?, created=?, expire=? WHERE `key`=?", data, now, expire, c.hash(key))
	} else {
		_, err = c.c.Exec("INSERT INTO cache(`key`,data,created,expire) VALUES(?,?,?,?)", c.hash(key), data, now, expire)
	}
	return err
}
...
func (c *MysqlCacher) read(key string) (*cache.Item, error) {
	...
	err := c.c.QueryRow("SELECT data,created,expire FROM cache WHERE `key`=?", c.hash(key)).Scan(&data, &created, &expire)
	if err != nil {
		return nil, err
	}
...
func (c *MysqlCacher) Delete(key string) error {
	_, err := c.c.Exec("DELETE FROM cache WHERE `key`=?", c.hash(key))
	return err
}
...
func (c *MysqlCacher) IsExist(key string) bool {
	var data []byte
	err := c.c.QueryRow("SELECT data FROM cache WHERE `key`=?", c.hash(key)).Scan(&data)
