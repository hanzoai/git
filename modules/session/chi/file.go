...
func (s *FileStore) Release() error {
	...
	return ioutil.WriteFile(s.p.filepath(s.sid), data, 0o600)
}
...
func (p *FileProvider) filepath(sid string) string {
	return filepath.Join(p.rootPath, filepath.Join(string(sid[0]), string(sid[1]), sid))
}
...
func (p *FileProvider) Exist(sid string) (bool, error) {
	p.lock.RLock()
	defer p.lock.RUnlock()
	stat, _ := os.Lstat(p.filepath(sid))
	return stat != nil, nil
}
...
func (p *FileProvider) regenerate(oldsid, sid string) (err error) {
	...
	if stat, _ := os.Lstat(oldname); stat == nil {
		data, err := EncodeGob(make(map[interface{}]interface{}))
		if err != nil {
			return err
		}
		if err = os.MkdirAll(filepath.Dir(oldname), 0o700); err != nil {
			return err
		}
		if err = ioutil.WriteFile(oldname, data, 0o600); err != nil {
			return err
		}
	}
...
func (p *FileProvider) Count() (int, error) {
	...
	if err := filepath.Walk(p.rootPath, func(path string, fi os.FileInfo, err error) error {
		...
		return 0, fmt.Errorf("error counting session files: %w", err)
	}
	return count, nil
}
...
func (p *FileProvider) GC() {
	...
	if stat, _ := os.Stat(p.rootPath); stat == nil || !stat.IsDir() {
		return
	}
