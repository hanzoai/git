...
test('register with mismatched passwords shows error', async ({page}) => {
  ...
  await page.getByLabel('Email Address').fill(`e2e-register-mismatch@${env.GIT_TEST_E2E_DOMAIN}`);
...
test('register then login', async ({page}) => {
  ...
  const email = `${username}@${env.GIT_TEST_E2E_DOMAIN}`;
...
test('register with existing username shows error', async ({page}) => {
  await page.getByLabel('Username').fill(env.GIT_TEST_E2E_USER);
  await page.getByLabel('Email Address').fill(`e2e-duplicate@${env.GIT_TEST_E2E_DOMAIN}`);
  await page.getByLabel('Password', {exact: true}).fill('password123!');
