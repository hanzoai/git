...
func runListAccessTokens(ctx context.Context, c *cli.Command) error {
	...
	// The test harness builds its own database, so initDB there re-enters
	// MustInstalled and kills the process before the command runs. Every other
	// admin command guards it the same way.
